...
const clickedOnOtherQueriesButton = ()=>{
    addBotMessage("Please let me know your question or the information you're looking for.")
    // Add textarea with a send button
    addBotTextInput();
}

const clickedOnJustBrowsingButton = ()=>{
    addBotMessage("No problem! Feel free to explore our website. If you need any assistance, I'm always here to help.")
    // Add another message with radio buttons
    setTimeout(() => {
        addBotMessage('Would you like to share your contact details for future assistance?')
        radioButtonHTML = 
            `<div class='radio-div'>
        ...
        $("input[name='contact-option']").on("change", function () {
            ...
            if (selectedOption === "Yes") {
                loading();
                setTimeout(()=>{
                    addBotMessage('Please provide your Email-Id or Contact Number so that we can reach out.')
                    addBotTextInput();
                }, 1300)
...
const clickedOnYourServicesButton = () => {
    ...
    <div class='dropdown-div'>
        ...
        ${services.map((service, index) => `<option value="${service.value}">${service.label}</option>`).join('')}
        </select>
...
const handleUserInput = (type='') => {
    let userInput = '';
    if (type == 'dropdown'){
        userInput = `service selected by you: <span class='bold'>${$("#service-dropdown").val()}<span>`
        $('.dropdown-div').remove();       
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            setTimeout(() => {
                ...
                addBotMessage(response.message || 'I received your Response! please allow me sometime..');
